...
- Largest P-value at the bottom. xxx
...
- Do not learn about “stepwise regression”! ref! xxx
- R has function `step` that does backward selection, like this:
```{r, eval=F}
step(rut.2, direction = "backward", test = "F")
```

Gets same answer as we did (by removing least significant x). xxx
...
less checks out with out scatterplots against `log.viscosity`. xxx
