...
\small
```{r}
(d <- tibble(x1 = 1:4, x2 = c(8, 7, 6, 5), 
             x3 = c(2, 4, 6, 9), y = my_df$y))
```
\normalsize
...
- R-like way: pivot-longer and rowwise.
...
## Get all the $x$-variables in one column

```{r}
d %>% pivot_longer(starts_with("x"), 
...
d %>% pivot_longer(starts_with("x"),
  ...
  nest_by(xname) %>%
  rowwise() %>%
  mutate(this_slope = with(data, slope(data$x, data$y)))
```
...
```{r}
d %>% summarize(across(everything(), ~ quartiles(.)))
```



## Write a function first and then do it rowwise

- If the “for each” part is simple, go ahead and use `rowwise`.
...
tibble(start = 1:100) %>%
  rowwise() %>% 
  mutate(seq_length = length(hotpo_seq(start))) %>%
