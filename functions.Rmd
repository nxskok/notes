...
Check by looking at the summary output from the regression: xxx
```{r}
tidy(yy.1)
```
...
- One of the things `lm` will accept is a data frame to take columns from. Let's make a little one:
...
- If the “for each” part is simple, go ahead and use `map_`-whatever. xxx
...
## What happens if we save the entire sequence? xxx
...
## Using the whole sequence to find its length and its maximum xxx
