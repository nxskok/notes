...
The mean of each column, with the columns labelled. Or:
...
- A list. Better: pretend output from quartiles is one-column data
frame:
```{r}
map_df(d,~quartiles(.))
```
...
- Strategy: it’s a loop, but I don’t know how long it’s going to be. I
keep looping “until `x` is 1”, or “while `x` is not 1”.
- For each new `x` I get, add it to the end of `ans`. When I hit 1, I break
...
arrange(desc(seq_length)) %>% slice(1:6)
```
...
## Using the whole sequence to find its length and its maximum

```{r}
tibble(start=1:10) %>%
mutate(sequence=map(start,hotpo_seq)) %>%
mutate(seq_length=map_int(sequence,~length(.)),
...
- [A pretty tree:](https://en.wikipedia.org/wiki/Collatz_conjecture#/media/File:Collatz-graph-all-30-no27.svg) 
