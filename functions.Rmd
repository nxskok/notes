...
- Inputs: one, a data frame, that contains columns called `x` and `y`.
...
```{r}
my_df
slope(my_df, subset = 3:4)
```


- Just uses the last two observations in xx and yy, so the slope should
be $(14 − 10)/(4 − 3) = 4$ and is.

## What if the columns are not called `x` and `y`?

- Requiring dataframe columns to be called `x` and `y` limiting: they could easily be called something else and then our function would not work.
- Use device known as "curly-curly":

```{r}
slope <- function(d, yname, xname, ...) {
  y.1 <- lm({{yname}} ~ {{xname}}, data = d, ...)
  tidy(y.1) %>% pluck("estimate", 2)
}
```

- Have *names* of columns to play roles of `y` and `x` as *inputs*
- In code, where we use those columns, wrap them in *two* curly brackets.

## Testing this

Make up another example where columns have different names:

```{r}
d <- tibble(u = 1:3, v = c(10, 11, 13))
d.1 <- lm(v ~ u, data = d)
summary(d.1)
```
Right answer is 1.5. Then:

```{r}
with(d, slope(d, yname = v, xname = u))
```
...
```{r}
(d <- tibble(x1 = 1:4, x2 = c(8, 7, 6, 5), x3 = c(2, 4, 6, 9), y = my_df$y))
```
...
for (i in 1:3) {
  col_name <- names(d)[i]
  slopes[i] <- with(d, slope(d, yname = y, xname = col_name))
}
slopes
```
