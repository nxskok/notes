...
- from [this link](https://raw.githubusercontent.com/nxskok/notes/master/test1.xlsx)
- open in Excel or other spreadsheet software (if it doesn't already open)
...
- `read_csv` guesses what kind of thing is in each column. 
- Here it correctly guesses that:
    - `id` and `group` are text (categorical variables). `id` is actually “identifier
variable”: identifies individuals.
    - `x` and `y` are possibly decimal numbers. 
...
load or save it). Find your `.csv` file, the location of which will be
saved in `f`, and then:
...
- Get the file yourself from [here](https://raw.githubusercontent.com/nxskok/notes/master/coffee.txt).
- This file was on my computer so I uploaded it to `rstudio.cloud`
 first.
...
- Here is [my spreadsheet](https://raw.githubusercontent.com/nxskok/notes/master/test2.xlsx) from before, but tarted up a bit:
