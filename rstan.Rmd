...
```{r}
ggplot(rats, aes(x=density, colour=group_fct))+geom_density()
```
...
The three elements of `mu`, and also `sigma`:
...
- The sampled values for each of the parameters should move freely across their posterior distributions (and not get stuck anywhere). This appears to have happened.
...
cbind(anova_ext$mu, sigma=anova_ext$sigma) %>% 
  ...
  mutate(group=fct_recode(group, Control="V1", Lowjump="V2", Highjump="V3")) -> sims
sims %>% sample_n(6)
```
...
## Compare posterior predicted distribution with actual data
...
```{r}
my_binwidth=15
ggplot(ppd, aes(x=sim_data)) + geom_histogram(binwidth=my_binwidth) +
  geom_dotplot(data=rats, aes(x=density), binwidth=my_binwidth) + 
  facet_wrap(~group) +
  scale_y_continuous(NULL, breaks = NULL) -> g 
```
